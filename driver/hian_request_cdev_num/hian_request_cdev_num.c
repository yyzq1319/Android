...
#define DEVICE_NAME "hian_sscdev"
...
static int hian_sscdev_init(void)
{
	int ret = 0;
	dev_t num_dev;
...
static void hian_sscdev_exit(void)
{
	printk(KERN_EMERG "HIAN CCSDEV exit!\n");
	
	unregister_chrdev_region(MKDEV(numdev_major,numdev_minor),DEVICE_MINOR_NUM);	
}

module_init(hian_sscdev_init);
module_exit(hian_sscdev_exit);
