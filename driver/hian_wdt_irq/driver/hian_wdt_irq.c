...
{
	wdt->wdtcon = (99 << 8) | (2 << 3) | (1<<2); //看门狗产生中断

	wdt->wdtcnt = 15625 * 3; //3s
	
	iowrite32(wdt->wdtcon, wdt->v + WTCON);
	iowrite32(wdt->wdtcnt, wdt->v + WTCNT);
	iowrite32(wdt->wdtcnt, wdt->v + WTDAT);
...
irqreturn_t wdt_isr(int irq, void *data)
{
	...
	printk(KERN_EMERG "wdt irq isr......\r\n");
	writel(1, wdt->v + WTCLEARINT); //任意值可清中断
...
static __init int scdev_init(void)
{
	...
	//申请看门狗中断
	ret = request_irq(IRQ_WDT, wdt_isr, 0, "hian_wdt_irq", wdt);    //第一个参数为中断线，在irqs.h中可以查到，第三个参数触发方式，0为不触发
	if(ret < 0)
	{
		clk_disable(wdt->clk);
		clk_put(wdt->clk);
