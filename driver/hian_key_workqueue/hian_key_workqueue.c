...
{
	printk("interrupt9 work.......!!!\r\n");
	return;	
}

static void dwork_handler(struct work_struct *dwork)
{
	printk("interrupt10 dwork.......!!!\r\n");
	return;	
}
...
static irqreturn_t hian_key_eint10_interrupt(int irq,void *dev_id)
{
	...
	schedule_delayed_work(&dwork, HZ);  //延时1s进入work队列
...
static int hian_key_irq_remove(struct platform_device *pdev)
{
	...
	flush_scheduled_work(); //等待所有的睡眠都完成
	return 0;
}
