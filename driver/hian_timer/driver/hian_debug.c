...
{
	printk(KERN_EMERG "data = %lu\r\n", data);
	if(in_interrupt()) //判断是否在中断上下文，timer在中断上下文
	{
		printk("interrupt\n");
	}
	else
	...
	timer.expires = jiffies + 2*HZ; //定时2s
	add_timer(&timer);
...
#if 0
	//定时器初始化方法2
	setup_timer(&timer, timer_hander, 456);
	timer.expires = jiffies + 2*HZ; //定时2s
	add_timer(&timer);
#endif
	//定时器初始化方法3
	setup_timer(&timer, timer_hander, 789);
...
static __exit void scdev_exit(void)
{
	...
	iounmap(debug->v);
	#endif
	del_timer_sync(&timer); //等待timer到来再卸载模块
	/*除去字符设备*/
	cdev_del(&(debug->dev));
	/*摧毁设备节点函数d*/
